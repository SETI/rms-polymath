...
class QubePickler:
    ...
    def set_pickle_digits(self, digits='double', reference='fpzip'):
        ...
        reference = QubePickler._validate_pickle_reference(reference)
        digits = QubePickler._validate_pickle_digits(digits, reference)
    ...
    def set_default_pickle_digits(digits='double', reference='fpzip'):
        ...
        _DEFAULT_PICKLE_DIGITS = QubePickler._validate_pickle_digits(digits, reference)
    ...
    def _check_pickle_digits(self):
        ...
        self._pickle_digits = QubePickler._validate_pickle_digits(digits, reference)
    ...
    def _encode_one_float_array(values, digits, reference):
        ...
        if reference == 'fpzip':
            (fpzip_bytes, bits) = QubePickler.fpzip_compress(values, digits=digits)
        ...
        if nbytes > 6:
            (fpzip_bytes, bits) = QubePickler.fpzip_compress(values)
        ...
        if nbytes == 4 and digits <= _SINGLE_DIGITS:
            (fpzip_bytes, bits) = QubePickler.fpzip_compress(values, dtype=np.float32)
    ...
    def _encode_floats(values, rank, digits, reference):
        ...
        if item == ():
            return QubePickler._encode_one_float_array(values, digits, reference)
        ...
        for element in array:
            encoded.append(QubePickler._encode_one_float_array(element, digits, reference))
    ...
    def _decode_floats(encoded):
        ...
        if method in ('float32', 'float64', 'fpzip'):
            ...
            return QubePickler.fpzip_decompress(fpzip_bytes, shape, bits)
        ...
        if method == 'scaled':
            return QubePickler._decode_scaled_uints(encoded)
        ...
        if len(items) == 1:
            return QubePickler._decode_floats(items[0]).reshape(shape)
        ...
        for k, item in enumerate(items):
            values[k] = QubePickler._decode_floats(item)
        ...
        * ('INT', shape) if BZ2 compression of integers was performed.
        ...
        # Otherwise, _values is an array and not fully masked
            ...
            if mask_shape:
                ...
                clone._mask = QubePickler._encode_bools(clone._mask)
            ...
            if dtype == 'float':
                ...
                clone._values = QubePickler._encode_floats(
                    clone._values, rank=len(self._item),
            ...
            # Integers use straight BZ2-encoding
                ...
                clone._values = QubePickler._encode_ints(clone._values)
            ...
            # Booleans use BZ2-encoding of the packed bits
                ...
                clone._values = QubePickler._encode_bools(clone._values)
    ...
    def __setstate__(self, state):
        ...
        while mask_encoding:
            ...
            if method == 'BOOL':
                ...
                self._mask = QubePickler._decode_bools(self._mask, shape, size)
        ...
        while vals_encoding:
            ...
            if method == 'INT':
                (_, shape) = encoding
                self._values = QubePickler._decode_ints(self._values, shape)

            elif method == 'BOOL':
                (_, shape, size) = encoding
                self._values = QubePickler._decode_bools(self._values, shape, size)
                values_is_writable = True

            elif method == 'FLOAT':
                self._values = QubePickler._decode_floats(self._values)
                values_is_writable = True
