...
class QubeVectorOps:
    ...
    def _mean_or_sum(self, axis=None, *, recursive=True, _combine_as_mean=False):
        ...
        self._check_axis(axis, 'mean()' if _combine_as_mean else 'sum()')
        ...
        if isinstance(axis, numbers.Integral):
            new_axis = axis % self._ndims
        elif axis is None:
            new_axis = tuple(range(self._ndims))
        else:
            new_axis = tuple(a % self._ndims for a in axis)

        # If there's no mask, this is easy
        if not np.any(self._mask):
            obj = Qube(func(self._values, axis=new_axis), False, example=self)

        # Handle a fully masked object
        elif np.all(self._mask):
            obj = Qube(func(self._values, axis=new_axis), True, example=self)

        # If we are averaging over all axes, this is fairly easy
        elif axis is None:
            if self._shape:
                obj = Qube(func(self._values[self.antimask], axis=0), False, example=self)
            else:
                obj = self
        ...
        # must be an array. Also, there must be at least one unmasked value.
            ...
            if np.any(new_mask):
                new_values[(new_mask,) +
                           self._rank * (slice(None),)] = self._default
            else:
                new_mask = False

            obj = Qube(new_values, new_mask, example=self)

        # Cast to the proper class
        obj = obj.cast(type(self))

        # Handle derivatives
        if recursive and self._derivs:
            new_derivs = {}
            for key, deriv in self._derivs.items():
                new_derivs[key] = deriv._mean_or_sum(axis, recursive=False,
    ...
    def _check_axis(self, axis, op):
        ...
        for i in axis:
            ...
            if selections[i]:
                raise IndexError(f'duplicated axis in {type(self)}.{op}: {axis_for_show}')
    ...
    def dot(arg1, arg2, axis1=-1, axis2=0, *, classes=(), recursive=True):
        ...
        if arg1._drank and arg2._drank:
            Qube._raise_dual_denoms('Qube.dot()', arg1, arg2)
        ...
        if a2 < 0 or a2 >= arg2._nrank:
            raise ValueError(f'second axis is out of range ({-arg2._nrank},{arg2._nrank}) '
                             f'in type(arg2).Qube.dot(): {axis2}')
        k2 = a2 + arg2._ndims

        # Confirm that the axis lengths are compatible
        if arg1._numer[a1] != arg2._numer[a2]:
            raise ValueError(f'{type(arg1)}.Qube.dot() axes have different lengths: '
    ...
    def norm_sq(arg, axis=-1, *, classes=(), recursive=True):
        ...
        arg._disallow_denom('Qube.norm_sq()')
    ...
    def cross(arg1, arg2, axis1=-1, axis2=0, *, classes=(), recursive=True):
        ...
        # At most one object can have a denominator.
        if arg1._drank and arg2._drank:
            Qube._raise_dual_denoms('Qube.cross()', arg1, arg2)
        ...
        if a1 < 0 or a1 >= arg1._nrank:
            raise ValueError(f'first axis is out of range ({-arg1._nrank},{arg1._nrank}) '
                             f'in {type(arg1)}.Qube.cross(): {axis1}')
        k1 = a1 + arg1._ndims
        ...
        if a2 < 0 or a2 >= arg2._nrank:
            raise ValueError(f'second axis is out of range ({-arg2._nrank},{arg2._nrank}) '
                             f'in {type(arg2)}.Qube.cross(): {axis2}')
        k2 = a2 + arg2._ndims

        # Confirm that the axis lengths are compatible
        if (arg1._numer[a1] != arg2._numer[a2]) or (arg1._numer[a1] not in {2, 3}):
            raise ValueError(f'invalid axis length for {type(arg1)}.Qube.cross(): '
    ...
    def outer(arg1, arg2, classes=(), recursive=True):
        ...
        if arg1._drank and arg2._drank:
            Qube._raise_dual_denoms('Qube.outer()', arg1, arg2)
