...
def test_qube_ext_pickler_invalid_digits_value_is_rejected() -> None:
    """An invalid string digit value names the full digits argument in the error."""

    a = Scalar([1., 2., 3.])
    with pytest.raises(ValueError,
                        match=r'invalid pickle digits value "\(\'double\', '
                              r'\'quadruple\'\)"'):
...
def test_qube_ext_pickler_invalid_reference_names_the_offending_value() -> None:
    ...
    a = Scalar([1., 2., 3.])
    with pytest.raises(ValueError, match='invalid pickle reference value "bogus"'):
