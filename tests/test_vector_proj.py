...
def test_vector_proj_single_values() -> None:
    """Single values."""

    np.random.seed(1634)

    assert Vector((2,3,0)).proj((0,7,0)) == (0,3,0)
    assert Vector((2,3,0)).proj((-1,0,0)) == (2,0,0)
    assert Vector((2,3,0),True).proj((-1,0,0)).mask
    assert Vector((2,3,0)).proj((0,0,0)).mask
    assert Vector((0,0,0)).proj((1,1,1)).norm() == 0.


def test_vector_proj_arrays_and_masks() -> None:
    """Arrays and masks."""
    ...
    z = y.proj(x)
    for i in range(N):
        assert z[i].cross(x[i]).norm() == 0. or abs(z[i].cross(x[i]).norm() - 0.) <= 1.e-14
        assert (y[i] - z[i]).dot(x[i]) == 0. or abs((y[i] - z[i]).dot(x[i]) - 0.) <= 1.e-14
    N = 100
    ...
    for i in range(len(zz)):
        assert zz[i].cross(xx[i]).norm() == 0. or abs(zz[i].cross(xx[i]).norm() - 0.) <= 1.e-14
        assert (yy[i] - zz[i]).dot(xx[i]) == 0. or abs((yy[i] - zz[i]).dot(xx[i]) - 0.) <= 1.e-14


def test_vector_proj_test_units() -> None:
    """Test units."""
...
def test_vector_proj_derivatives_denom() -> None:
    """Derivatives, denom = ()."""
...
def test_vector_proj_derivatives_denom_2() -> None:
    """Derivatives, denom = (2,)."""
    ...
    assert y.proj(x, recursive=False).derivs == {}
    assert hasattr(x, 'd_df')
    assert hasattr(x, 'd_dh')
    assert hasattr(y, 'd_dg')
    assert hasattr(y, 'd_dh')
    assert not hasattr(y.proj(x, recursive=False), 'd_df')
    assert not hasattr(y.proj(x, recursive=False), 'd_dg')
    assert not hasattr(y.proj(x, recursive=False), 'd_dh')


def test_vector_proj_read_only_status_should_not_be_preserved() -> None:
    ...
    assert not y.proj(x).readonly
    assert x.as_readonly().readonly
    assert y.as_readonly().readonly
    assert not y.as_readonly().proj(x.as_readonly()).readonly
    assert not y.as_readonly().proj(x).readonly
    assert not y.proj(x.as_readonly()).readonly
