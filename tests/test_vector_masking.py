...
def test_vector_masking() -> None:
    ...
    # clip_component(), etc.
    ############################################################################################
    assert a.clip_component(2,2,8, remask=False) == [[0,1,2],[3,4,5],[6,7,8]]
    assert a.clip_component(2,2,7, remask=False) == [[0,1,2],[3,4,5],[6,7,7]]
    assert a.clip_component(2,2,6, remask=False) == [[0,1,2],[3,4,5],[6,7,6]]
    assert a.clip_component(2,2,3, remask=False) == [[0,1,2],[3,4,3],[6,7,3]]
    assert a.clip_component(2,2,None, remask=False) == [[0,1,2],[3,4,5],[6,7,8]]
    assert a.clip_component(2,None,3, remask=False) == [[0,1,2],[3,4,3],[6,7,3]]
    assert a.clip_component(2,2,8, remask=True) == [[0,1,2],[3,4,5],[6,7,8]]
    assert np.all(a.clip_component(2,2,7, remask=True).mask == mask001)
    assert np.all(a.clip_component(2,2,6, remask=True).mask == mask001)
    assert np.all(a.clip_component(2,2,3, remask=True).mask == mask011)
    assert np.all(a.clip_component(2,2,None, remask=True).mask == mask000)
    lower = Scalar([4,3,2])
    upper = Scalar([5,4,3],mask=[0,1,0])
    assert a.clip_component(2,lower,upper, remask=False) == [[0,1,4],[3,4,5],[6,7,3]]
