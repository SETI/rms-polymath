...
def test_qube_getstate_scalar_tests_derivatives_units() -> None:
                ...
                for derivs in ({}, {'t': deriv1}, {'xy': deriv2},
                  ...
                  for key in a.derivs:
                    ...
                    if Qube.DEFAULT_PICKLE_DIGITS[1] == 'single':
                        ...
                        assert (diffs[antimask].rms() < 3.e-7).all()
                    else:
                        assert a.derivs[key][antimask] == b.derivs[key][antimask]
                    assert readonly == b.derivs[key].readonly
            ...
            for mask in (mask1, mask2):
              ...
              for derivs in ({}, {'t': deriv1}, {'xy': deriv2},
                ...
                for key in a.derivs:
                    ...
                    if Qube.DEFAULT_PICKLE_DIGITS[1] == 'single':
                        diffs = a.derivs[key] - b.derivs[key]
                        assert (diffs[antimask].rms() < 3.e-7)
                    else:
                        assert a.derivs[key][antimask] == b.derivs[key][antimask]
                    assert readonly == b.derivs[key].readonly
    ...
    for digits in np.arange(6.5, 17.5, 0.5):
        ...
        for reference, ref_value in zip(references, ref_values, strict=False):
            ...
            if digits < 7:
                assert (max_error <= ref_value * error * 1.5)
            else:
                assert (max_error <= ref_value * error)

            # Mean error should be ~ max_error/100 for N = 10,000
            max_mean = 4. * max_error/100.
            assert ((a - b).mean(builtins=True) <= max_mean)
