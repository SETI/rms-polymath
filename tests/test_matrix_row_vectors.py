...
def test_matrix_row_vectors_check_unit_and_masks() -> None:
    ...
    b = a.row_vector(0)
    assert a.shape == b.shape
    assert a.values.shape == (N,2,3)
    assert b.values.shape == (N,3)
    assert type(b) == Vector3
    assert type(a.row_vector(0, classes=Vector)) == Vector
    c = a.row_vectors()
    assert a.shape == c[0].shape
    assert b == c[0]
    ...
    b[0].values[0] = 22.
    assert a[0].values[1,0] == 22.


def test_matrix_row_vectors_check_derivatives() -> None:
    """check derivatives."""
...
def test_matrix_row_vectors_read_only_status() -> None:
    """read-only status."""
